...
!!! note "Windows / macOS 공통 가이드"
    Spring Boot 프로젝트 자체와 Git Repository 구조는 운영체제에 종속되지 않는다.
    따라서 본 문서는 Spring Initializr 절차를 공통으로 설명하고,
    로컬 Directory, Shell 명령, 단축키처럼 운영체제에 따라 달라지는 부분만 Windows/macOS로 구분한다.

    개발환경의 실제 설치 방식은 각 운영체제별 **개발 환경 구성 가이드**에서 다룬다.
...
MicroServer 프로젝트는 운영체제와 관계없이 동일한 논리적 Directory 구조를 사용한다.

본 문서에서는 로컬 개발환경의 최상위 Directory를 다음과 같이 표현한다.

```text
<MICROSERVER_HOME>
```

운영체제별 기본 Root는 다음과 같다.

| 운영체제 | `<MICROSERVER_HOME>` |
|---|---|
| Windows | `C:\local-microserver` |
| macOS | `~/local-microserver` |

프로젝트 Source는 공통적으로 `workspace` Directory 아래에 둔다.
...
`gradlew.bat`은 Windows용 Wrapper이며 `gradlew`는 macOS/Linux에서 사용한다.
Git으로 프로젝트를 공유하므로 두 파일은 Repository에 함께 유지한다.

### 3.1 `workspace` Directory 생성

아직 `workspace` Directory가 없다면 운영체제에 맞는 Shell에서 생성한다.
...
=== "macOS"
    ...
    ```bash
    mkdir -p ~/local-microserver/workspace
    ```

    확인:

    ```bash
    ls -la ~/local-microserver
    ```
...
!!! important "`microserver` Directory는 미리 만들지 않음"
    ...
    생성 위치로 `<MICROSERVER_HOME>/workspace`를 선택한다.

    Initializr가 다음 구조를 직접 생성하도록 한다.
    ...
    ```text
    X <MICROSERVER_HOME>/workspace/microserver/microserver
    ```
...
MicroServer 개발환경의 VS Code를 실행한다.

=== "Windows"

    Portable 환경을 구성한 경우 다음 Shortcut을 사용하는 것을 기준으로 한다.
...
이후 Spring Initializr Wizard의 진행 절차는 Windows와 macOS가 동일하다.
...
현재 프로젝트 표준 Java Version은 `25`이다.

로컬 JDK의 실제 설치 경로는 운영체제별 개발환경 구성 가이드에서 정의하며,
Spring Initializr에서는 로컬 설치 위치와 관계없이 프로젝트 표준에 맞춰 Java `25`를 선택한다.
...
Dependency 선택이 끝나면 Folder 선택 창이 열린다.
...
따라서 Spring Initializr가 다음 공통 구조를 생성하도록 하는 것이 목표이다.
...
!!! warning "Directory 중첩 주의"
    ...
    ```text
    O <MICROSERVER_HOME>/workspace/microserver/build.gradle
    ```

    잘못된 구조:

    ```text
    X <MICROSERVER_HOME>/workspace/microserver/microserver/build.gradle
    ```
...
=== "macOS"
    ...
    ```bash
    ls -la ~/local-microserver/workspace/microserver
    ```
...
- [ ] `<MICROSERVER_HOME>/workspace`가 존재한다.
...
- [ ] 생성 위치는 `<MICROSERVER_HOME>/workspace`이다.
- [ ] `<MICROSERVER_HOME>/workspace/microserver`가 생성되었다.
