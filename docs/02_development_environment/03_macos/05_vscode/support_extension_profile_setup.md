# macOS 개발 지원 Extension 및 Profile 구성
...
```bash
$code = "~/local-microserver/tools/vscode/Visual Studio Code.app/Contents/Resources/app/bin/code"
...
`start-vscode.command` 또는 환경 초기화 Script로 Portable VS Code의 `bin`이 PATH에 포함된 경우에는 다음처럼 실행할 수 있다.
...
Portable Mode의 Profile 관련 User Data는 VS Code가 `code-portable-data/user-data` 영역에서 관리한다.
...
MicroServer macOS 개발환경의 목표는
VS Code를 포함한 주요 개발도구를 `~/local-microserver` 아래에 모아
