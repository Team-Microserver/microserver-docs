...
```text
VS Code 자체 실행
→ Visual Studio Code.app / Electron Runtime
...
`Visual Studio Code.app`의 JVM을 지정하는 것이 아니라 **Java Extension과 개발 Terminal이
MicroServer의 JDK / Gradle을 일관되게 찾을 수 있게 만드는 것**이다.

### 3.2 `start-vscode.command`에서 Session 환경 주입
...
start "" "%LOCAL_MICROSERVER%\tools\vscode\Visual Studio Code.app"
```
...
```mermaid
flowchart LR
    SCRIPT["start-vscode.command"]
    --> ENV["JAVA_HOME / GRADLE_HOME<br/>GRADLE_USER_HOME / PATH"]
    --> CODE["Portable Visual Studio Code.app"]
    --> JAVA["Java Extension"]
...
| 구분 | 역할 | 현재 기준 |
|---|---|---|
| 시스템 전역 `JAVA_HOME` | macOS 전체 기본 Java | 프로젝트용으로 영구 고정하지 않음 |
| Session `JAVA_HOME` | `start-vscode.command`에서 시작되는 VS Code / Terminal Bootstrap | Temurin 25 사용 가능 |
| Workspace JDK | 실제 MicroServer Project의 Java Runtime | 프로젝트 생성 이후 명시적으로 설정 |

즉 `start-vscode.command`의 `JAVA_HOME`은
...
따라서 `start-vscode.command`의 JDK / Gradle 경로를 변경한 뒤에는
다음 순서를 권장한다.
...
`gradlew`이나 프로젝트 Build는 실행하지 않는다.
...
- [VS Code 설치 및 기본 설정](vscode_install_basic_setup_legacy.md)
- [프로젝트 JDK / VS Code Workspace 설정](../../03_project_creation_verification/02_project_environment/project_jdk_vscode_setup.md)
...
Session JAVA_HOME
→ start-vscode.command에서 현재 VS Code Process에 전달하는 Java
...
- [ ] `start-vscode.command`를 이용한 Session 환경 주입 방식을 이해했다.
- [ ] VS Code가 정상 설치되어 있다.
