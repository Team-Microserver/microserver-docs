...
Shortcut, 실행 Script, Portable `code-portable-data` 구성은 이전
...
Portable Mode에서는 User Settings가 일반 macOS 기본 User Data Directory이 아니라 Portable `code-portable-data` 영역에서 관리된다.
