...
Windows에서는 Terminal, macOS에서는 zsh를 기본으로 사용할 수 있다.
...
!!! tip "바탕화면 바로가기 사용"
    `~/Desktop/MicroServer VS Code.command`를
...
!!! note "Terminal Script 실행이 차단되는 경우"
    회사 보안정책이나 Terminal 실행정책으로 인해 바로가기를 통한 Script 실행이 차단될 수 있다.

    본 가이드에서는 정책 변경 또는 우회 방법을 별도로 제공하지 않는다.

    **권한 또는 Script 실행 문제가 발생하면 개발환경을 배포하거나 운영하는 팀에 문의한다.**

### 6.2 `Visual Studio Code.app` 직접 실행과의 차이
...
하지만 `Visual Studio Code.app`를 직접 실행하면
`start-vscode.command`에서 구성하는 Local 환경변수를 거치지 않는다.
...
!!! info "Portable Mode의 User Settings 저장 위치"
    ...
    해당 Settings도 `~/local-microserver` Package와 함께 전달할 수 있다.
...
먼저 JDK가 정상적으로 설치되어 있는지 Terminal에서 확인한다.
...
!!! important "JDK Home을 지정한다"
    ...
    ```text
    ~/local-microserver/tools/jdk/temurin-25.jdk/Contents/Home\bin
    ```
...
- [ ] `~/local-microserver/tools/jdk/temurin-25.jdk/Contents/Home`에 JDK가 설치되어 있음을 확인했다.
