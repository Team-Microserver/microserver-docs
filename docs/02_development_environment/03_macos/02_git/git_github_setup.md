# macOS Git / GitHub 환경 구성

## 1. 문서 목적

이 문서는 Apple Silicon macOS 개발장비에서 Git과 GitHub를 구성하고,
`~/local-microserver/workspace` 아래의 각 프로젝트 Repository를 독립적으로 관리하는 기준을 정의한다.

macOS에서 Git은 개발환경 Package 내부에 강제로 포함하지 않고,
Xcode Command Line Tools 또는 회사가 관리하는 Git을 사용하는 것을 기본으로 한다.
...
필요한 경우 macOS에서 Xcode Command Line Tools 설치가 안내될 수 있다.
...
`~/local-microserver` 자체에는 `.git`을 생성하지 않는다.
...
Repository의 `.gitattributes`가 있으면 해당 정책을 우선한다.

## 1.4 GitHub 인증

HTTPS 또는 SSH를 사용할 수 있다. SSH Private Key와 GitHub Token은 공용 개발환경 Package에 포함하지 않는다.
...
MicroServer macOS 개발환경은 `~/local-microserver` 아래에
개발도구, 실행 Script, Workspace를 함께 배치하지만
**`~/local-microserver` 전체를 하나의 Git Repository로 관리하지 않는다.**
...
```text
~/local-microserver/.git
~/local-microserver/workspace/.git
```
...
### 3.3 `~/local-microserver`에 `.gitignore`가 필요한가?

**현재 표준 구조에서는 필요하지 않다.**

이유는 `~/local-microserver` 자체가 Git Repository가 아니기 때문이다.
...
|---|---:|---:|
| `~/local-microserver` | 아니오 | 필요 없음 |
| `~/local-microserver/workspace` | 아니오 | 필요 없음 |
| `~/local-microserver/workspace/microserver` | 예 | 필요 |
| `~/local-microserver/workspace/microserver-docs` | 예 | 필요 |

!!! note "`local-env.cmd`도 Root `.gitignore`로 막을 필요가 없음"
    다음 파일은:

    ```text
    ~/local-microserver/env/local-env.cmd
    ```
...
하지만 `~/local-microserver` 전체를 직접 ZIP으로 압축하면
...
~/local-microserver 개발환경 Package
...
```text
배포 포함
~/local-microserver/env/local-env.example.cmd
...
```text
~/local-microserver/workspace/microserver/.gitignore
```

기술문서가 별도 Repository라면 해당 Repository에도 별도의 `.gitignore`를 둔다.

```text
~/local-microserver/workspace/microserver-docs/.gitignore
```
...
- [ ] `~/local-microserver` Root가 Git Repository가 아님을 확인했다.
- [ ] `workspace`는 Repository를 보관하는 상위 Directory임을 이해했다.
- [ ] 각 프로젝트 Directory에 개별 `.git`이 존재하는 구조를 사용한다.
- [ ] MicroServer 저장소를 `~/local-microserver/workspace` 아래에 Clone했다.
- [ ] `.gitignore`는 각 Repository Root에서 관리한다.
- [ ] `~/local-microserver` Root에는 `.gitignore`가 필요하지 않음을 이해했다.
