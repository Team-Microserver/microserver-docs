...
macOS 개발환경에서는 앞 단계에서 정의한 다음 Root Directory를 기준으로 JDK를 관리한다.
...
예를 들어 하나의 개발 장비에서 다음과 같이 서로 다른 프로젝트를 운영할 수 있다.
...
새로운 JDK를 추가할 때 기존 JDK를 덮어쓰기보다 별도 Directory에 준비하고,
필요한 프로젝트나 개발도구에서 사용할 JDK를 선택하도록 구성한다.
...
!!! tip "JDK 설치와 JDK 연결은 구분"
    ...
    일반 Terminal에서 사용할 `setup.command` / `setup.cmd` 역시
...
macOS에서는 Apple Silicon과 Intel Architecture에 따라 Temurin Package를 선택하고 TAR.GZ 압축 배포본을 사용한다.

실제 다운로드, `.jdk` Bundle 구조, `Contents/Home` 확인 및 실행 검증은 다음 가이드에서 진행한다.

**[macOS JDK 설치 및 검증](macos_jdk_setup.md)**

!!! note "macOS JDK 보관 경로"

    macOS에서는 현재 다음 JDK 보관 경로를 사용한다.

    ```text
    ~/local-microserver/tools/jdk
    ```

    macOS 프로젝트 로컬 개발환경 Root가 별도로 표준화되면 관련 경로도 함께 조정한다.
...
| macOS 배포 형식 | ZIP |
| macOS 배포 형식 | TAR.GZ |
| macOS JDK Root | `~/local-microserver/tools/jdk` |
...
- [ ] 프로젝트 표준 JDK Home이 `~/local-microserver/tools/jdk/temurin-25.jdk/Contents/Home`임을 확인했다.
