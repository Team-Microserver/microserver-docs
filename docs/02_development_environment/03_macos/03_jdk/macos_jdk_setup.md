# macOS JDK 설치 및 검증 - Apple Silicon
...
!!! note "macOS 경로 기준"
    ...
    macOS에서는 기존 개발도구 보관 경로인 `~/local-microserver/tools/jdk` 체계를 사용한다.
...
```bash
mkdir -p ~/local-microserver/tools/jdk
```
...
다운로드한 TAR.GZ 파일을 `~/local-microserver/tools/jdk`에 압축 해제한다.
...
- [ ] JDK를 `~/local-microserver/tools/jdk` 계열 경로에 압축 해제했다.
