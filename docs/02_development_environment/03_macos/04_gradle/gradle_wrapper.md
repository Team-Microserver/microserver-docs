# macOS Gradle Wrapper 개념 및 운영 원칙
...
MicroServer의 macOS 개발환경에서는 개발자 사용자 Home에 대한 의존성을 줄이기 위해
...
!!! info "현재는 Build 명령을 실행하지 않음"
    ...
    `./gradlew build` 또는 `.\gradlew build`를 실행하지 않는다.
...
!!! warning "현재 단계에서는 실행하지 않음"
    ...
    `gradlew`와 `gradlew` 파일이 존재하지 않는다.
    ...
    `gradlew` / `gradlew`을 이용한 Wrapper Version 확인은
...
- [ ] `./gradlew --version` 또는 `.\gradlew --version`이 정상 동작한다.
