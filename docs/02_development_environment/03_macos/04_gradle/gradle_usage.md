...
별도로 변경하지 않으면 일반적으로 사용자 Home의 `.gradle` Directory가 사용된다.
...
| 구분 | 예 | 역할 |
|---|---|---|
| `GRADLE_HOME` | `~/local-microserver/tools/gradle/gradle-9.7.1` | 개발 PC에 설치한 Gradle 프로그램 위치 |
| `GRADLE_USER_HOME` | `~/.gradle` | 사용자별 Cache / Wrapper Distribution / 설정 |
