# macOS Maven 설치 및 기본 환경 구성 - 비교 / 참고
...
```text
~/local-microserver/jdks/temurin-26.jdk/Contents/Home
```
...
```bash
mkdir -p ~/local-microserver/tools
```

Maven 압축을 해제한다.

```bash
tar -xzf apache-maven-3.9.16-bin.tar.gz -C ~/local-microserver/tools
```
