...
### 3.1 Java Extension에서 사용할 JDK 기준

Java Extension이 사용할 JDK Runtime은 이전 단계의
**[VS Code User Settings](vscode_user_settings.md)** 에서 등록한 값을 사용한다.
...
이 문서에서는 `java.configuration.runtimes`와
`java.jdt.ls.java.home` 설정을 다시 작성하지 않는다.

Java Extension 설치 후 JDK Runtime이 정상적으로 인식되는지만 확인한다.

!!! note "프로젝트가 아직 없어도 정상"
    현재 단계에서는 아직 Java / Spring Boot 프로젝트를 생성하지 않았으므로
    `Java: Configure Java Runtime` 실행 시
    `There are no Java projects opened in the current workspace.` 메시지가 나타날 수 있다.

    이는 JDK 설치 오류가 아니라 현재 Workspace에 Java 프로젝트가 없다는 의미이다.
