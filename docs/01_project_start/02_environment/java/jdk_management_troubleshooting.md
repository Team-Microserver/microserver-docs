...
```bash
test -f ~/dev/jdks/temurin-25.jdk/Contents/Home/bin/java && echo "OK"
```

!!! info "이후 개발환경에서는"
    일반 Terminal에서는 `setup.ps1` / `setup.cmd` 등을 통해 현재 Session의 `JAVA_HOME`과 PATH를 구성할 수 있고,
...
```text
~/dev/jdks/temurin-25.jdk/Contents/Home/bin/javac
```
...
```text
O  ~/dev/jdks/temurin-25.jdk/Contents/Home
X  ~/dev/jdks/temurin-25.jdk
X  ~/dev/jdks/temurin-25.jdk/Contents/Home/bin
```
...
- [ ] JDK Home은 `~/dev/jdks/temurin-25.jdk/Contents/Home`이다.
- [ ] `bin/java`가 정상 실행된다.
- [ ] `bin/javac`가 정상 실행된다.
